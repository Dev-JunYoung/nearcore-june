...
impl Client {
    ...
    pub fn produce_block(&mut self, next_height: BlockHeight) -> Result<Option<Block>, Error> {
        ...
        let next_epoch_id = self
            ...
            .expect("Epoch hash should exist at this point");
        println!("      next_epoch_id : {:#?}", next_epoch_id);
        ...
        println!("      _span : {:#?}", _span);
        println!("      known_height : {:#?}", known_height);
        println!("      head : {:#?}", head);
        println!("      next_block_proposer : {:#?}", next_block_proposer);
        println!("      prev : {:#?}", prev);
        println!("      prev_hash : {:#?}", prev_hash);
        println!("      prev_height : {:#?}", prev_height);
        println!("      prev_prev_hash : {:#?}", prev_prev_hash);
        println!("      prev_epoch_id : {:#?}", prev_epoch_id);
        println!("      prev_next_bp_hash : {:#?}", prev_next_bp_hash);
        println!("      validator_pk : {:#?}", validator_pk);
        println!("      approvals_map: {:#?}", approvals_map);
        println!("      protocol_version : {:#?}", protocol_version);
        println!("      protocol_version : {:#?}", protocol_version);
        println!("      gas_price_adjustment_rate : {:#?}", gas_price_adjustment_rate);
        println!("      min_gas_price : {:#?}", min_gas_price);
        println!("      max_gas_price : {:#?}", max_gas_price);
        println!("      next_bp_hash : {:#?}", next_bp_hash);
        println!("      timestamp_override : {:#?}", timestamp_override);
        println!("      timestamp_override : {:#?}", timestamp_override);
        println!("      block_merkle_tree : {:#?}", block_merkle_tree);
        println!("      block_merkle_root : {:#?}", block_merkle_root);
        println!("      block_ordinal: {:#?}", block_ordinal);
        println!("      prev_block_extra : {:#?}", prev_block_extra);
        println!("      prev_block : {:#?}", prev_block);
        println!("      prev_header : {:#?}", prev_header);
        println!("      minted_amount : {:#?}", minted_amount);
        println!("      epoch_sync_data_hash : {:#?}", epoch_sync_data_hash);
        println!("      challenges : {:#?}", self.challenges);
        println!("      this_epoch_protocol_version : {:#?}", this_epoch_protocol_version);
        println!("      next_epoch_protocol_version : {:#?}", next_epoch_protocol_version);
        println!("      block : {:#?}", block);
