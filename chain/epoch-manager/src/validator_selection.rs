...
print_file_path_and_function_name!();
    println!("--------------------------------------------------");
    println!("다음 에포크의 유효성 검사기를 선택하고 에포크 정보를 생성합니다.");
    println!("--------------------------------------------------");
    debug_assert!(
...
print_file_path_and_function_name!();
    ...
    select_validators(block_producer_proposals, max_num_selected, min_stake_ratio, protocol_version)//블록 생산자
}
...
println!("let mut total_stake = 0;
    ...
    for _ in 0..n {
        ...
        if Ratio::new(p_stake, total_stake_with_p) > min_stake_ratio {
            ...
            // OrderedValidatorStake(p):
            // 스테이크를 최대 힙에 저장하고 가장 큰 상태와 (동점인 경우)
            // 사전적으로 가장 작은 AccountId를 가진 유효성 검사기가 맨 위에 오도록 정렬하려고 합니다.
            // p was not included, return it to the list of proposals
            proposals.push(OrderedValidatorStake(p));
            break;
        }
    }
    //proposals.iter_mut().for_each(|p| println!("{:?}", p));
    println!("--------------------반복문 끝--------------------");
    if validators.len() == max_number_selected {
        // all slots were filled, so the threshold stake is 1 more than the current
        // smallest stake
        println!("모든 슬롯이 채워졌으므로 임계값 지분은 현재 가장 작은 지분보다 1 더 많습니다.");
