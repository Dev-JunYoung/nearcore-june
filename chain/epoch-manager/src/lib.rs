...
print_file_path_and_function_name!();
    ...
    /// is called
    /// 모의 노드에서만 사용 `block_hash`와 관련된 필요한 에포크 정보를 `source_epoch_manager`에서 현재 에포크 매니저로 복사합니다.
    /// 이 함수는 EpochInfoAggregator에 저장된 정보를 복사하지 않으므로,
    /// 이 함수를 호출한 후 에포크 매니저가 제대로 작동하려면 `block_hash`가 에포크의 마지막 블록이어야 합니다.
...
print_file_path_and_function_name!();
        ...
        println!("에포크의 첫 번째 블록에 대한 블록 정보 저장");
        println!("let epoch_first_block = block_info.epoch_first_block();");
