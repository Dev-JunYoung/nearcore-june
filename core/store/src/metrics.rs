...
print_file_path_and_function_name!();
    println!("tracing::debug!(target:\"metrics\", \"Spawning the db metrics loop.\");");
    ...
    db_metrics_arbiter.spawn(async move {
        ...
        println!("db_metrics_arbiter.spawn(async move {{
        tracing::debug!(target:\"metrics\", \"Starting the db metrics loop.\");");
        loop {
            interval.tick().await;
            export_store_stats(&hot_store, Temperature::Hot);
            println!("export_store_stats(&hot_store, Temperature::Hot);");
            if let Some(cold_store) = &cold_store {
                export_store_stats(cold_store, Temperature::Cold);
                println!("export_store_stats(cold_store, Temperature::Cold);");
            }
        }
    });
    println!("Ok(db_metrics_arbiter.handle())");
